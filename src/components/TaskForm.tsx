...
const TaskForm = ({ onTaskCreated }: TaskFormProps) => {  
  ...
  const [category, setCategory] = useState("personal"); // Garante valor padrão  
  ...
  const handleSubmit = async (e: React.FormEvent) => {  
    ...
    const { error } = await supabase  
      ...
      .insert([{  
        ...
        priority,  
        category, // Campo 'category' agora existe no banco  
        status: "pending",  
      }]);  
    ...
    <form onSubmit={handleSubmit} className="space-y-3 p-1">  
      ...
      <div className="flex flex-wrap items-center gap-2">  
        ...
        <Select value={priority} onValueChange={setPriority}>  
          <SelectTrigger  
            className="h-8 w-auto rounded-full text-[10px] font-bold uppercase tracking-wider px-3 border-none bg-transparent hover:bg-slate-100 dark:hover:bg-slate-800 shadow-none focus:ring-0"  
          >  
